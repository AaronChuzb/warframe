...
<template>
  <div class="m-image" :class="[{ showLoading, border }, `m-image-${index}`]" :style="{ '--bgColor': bgColor }">
    <image :class="{ showPic }" :mode="mode" :src="index === null ? src : url" lazy-load @load="showPic = true" />
  </div>
</template>
  ...
  mounted() {
    ...
    if (this.index !== null) {
      this.$nextTick(() => {
        ...
        // 这里是组件内获取布局状态，不能用uni.createIntersectionObserver，而必须用this.createIntersectionObserver(只在小程序生效)
        ...
        contentObserver
          ...
          .observe(`.m-image-${this.index}`, res => {
            ...
            if (res.intersectionRatio > 0) {
              // 懒加载状态改变
              this.url = this.src + '?x-oss-process=image/resize,p_50/quality,q_100/sharpen,100'
